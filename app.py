...
# 0-5 の 6 段階に対応するステップカラースケール
_MAP_COLORSCALE = [
    [0.000, "#cbd5e1"], [0.166, "#cbd5e1"],   # 0: データ未取得
    [0.167, "#f0fdf4"], [0.333, "#f0fdf4"],   # 1: 停電なし
    [0.334, "#fef08a"], [0.500, "#fef08a"],   # 2: 〜100軒
    [0.501, "#fbbf24"], [0.667, "#fbbf24"],   # 3: 〜1,000軒
    [0.668, "#f97316"], [0.833, "#f97316"],   # 4: 〜10,000軒
    [0.834, "#dc2626"], [1.000, "#dc2626"],   # 5: 10,000軒以上
]


@st.cache_data(ttl=86400 * 7, show_spinner=False)
def _fetch_japan_geojson() -> dict | None:
    """日本都道府県 GeoJSON を取得（週次キャッシュ）"""
    try:
...
def build_japan_map_fig(df: pd.DataFrame) -> "go.Figure | None":
    """地理的な日本地図コロプレス（Yahoo地図風）を返す"""
    ...
    df_map["hover_text"] = df_map.apply(
        lambda r: (
            f"<b>{r['prefecture']}</b><br>{r['outage_level']}"
    ...
    fig = px.choropleth_mapbox(
        ...
        zoom=4.0,
        center={"lat": 36.0, "lon": 136.5},
        opacity=0.72,
        hover_name="hover_text",
        hover_data={"level_num": False, "prefecture": False, "hover_text": False},
        height=520,
    )
    fig.update_layout(
